...
from classes.db.UrsaDatabase import UrsaDatabase as ursaDb
import cv2
...
##        self.conList = ['LVDS','VBY1','WIFI','DC','AC','FPC']
        ...
        self.Button1 = tk.Checkbutton(mbTk, text = "T-con",
    ...
    def detect_edge(self, image):
        ...
        mbCn = tk.Canvas(mbTk, width= self.imgSX, height = self.imgSY, background = 'white')
...
##            self.conCo.append([ev, self.conInt])
        ...
        if self.selB == 3 and not self.is_in_con_sel and not self.is_in_screw_finding:
            ...
            self.conCo.append([[tX/self.refCm,tY/self.refCmY], st.conList[self.conInt]])
            ...
            self.screwCo.append([x/self.refCm,y/self.refCmY])
    ...
    def test(self):
        tcS = int(float(self.Checkbutton1.get()))
        print(tcS)
        self.includes = [tcS]
        print(self.includes)
    ...
    def locateScrews(self):
        ...
        mbScFrCv.create_text(self.cm*(self.xS/2+1),self.cm*(self.yS/2+1), text = 'Ursa', angle=0)
        self.rects.append([self.cm, self.cm, (self.xS+1)*self.cm, (self.yS+1)*self.cm])
        self.texts.append([self.cm*(self.xS/2+1),self.cm*(self.yS/2+1),'Ursa'])
        self.cc = 0
        self.sc = 0
