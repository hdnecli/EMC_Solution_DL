...
from classes.db.PsuDatabase import PsuDatabase as psuDb
import cv2
...
##        self.conList = ['LVDS','VBY1','WIFI','DC','AC','FPC']
        ...
        self.Button1 = tk.Checkbutton(mbTk, text = "LED Driver",
...
##            self.conCo.append([ev, self.conInt])
    ...
    def test(self):
        ldS = int(float(self.Checkbutton1.get()))
        print(ldS)
        self.includes = [ldS]
        print(self.includes)
    ...
    def locateScrews(self):
        ...
        mbScFrCv.create_text(self.cm*(self.xS/2+1),self.cm*(self.yS/2+1), text = 'PSU', angle=0)
        self.cc = 0
        self.sc = 0
    ...
    def locateCons(self):
        ...
        mbCnFrCv.create_text(self.cm*(self.xS/2+1),self.cm*(self.yS/2+1), text = 'PSU', angle=0)
